...
| Version Tag | `.github/workflows/version.yml` | `main` push | `contents: write` | 根据 Conventional Commits 计算下一版本，更新 Cargo 版本并推送 `vX.Y.Z` tag。 |
| Release | `.github/workflows/release.yml` | `v*` tag push | gate/build 为 `contents: read`，publish 为 `contents: write` | 重新运行 Rust gate，校验 SemVer tag 与 `main` 祖先关系，构建多平台二进制并创建 GitHub Release。 |
...
`version.yml` 会在 `main` 上串行执行，并使用 `contents: write` 权限执行以下动作：
...
`release.yml` 只由 `v*` tag 触发，并在运行时要求 tag 严格匹配 `^v\d+\.\d+\.\d+$` 且 tag commit 是 `main` 的祖先。每个 tag 会构建并上传以下产物：
...
| `x86_64-apple-darwin` | `macos-13` | `port-mcp-vX.Y.Z-x86_64-apple-darwin.tar.gz` |
...
仓库需要允许 GitHub Actions 使用 `GITHUB_TOKEN` 写入 contents，才能让 `version.yml` 推送版本提交、tag 和 Release。如果 `main` 启用了分支保护，需要确认 GitHub Actions bot 是否允许写入，或改用受控 PAT 并更新 workflow token 配置。
...
实际推送版本提交、创建 tag、上传 Release artifact 需要在 GitHub Actions 环境中验证。首次启用后建议手工检查一次：版本提交是否落在 `main`、tag 是否指向版本提交、四个平台 artifact 是否完整上传、Release notes 是否符合预期。
